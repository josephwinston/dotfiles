...
;;
;; $Id: //depot/Jody/main/jody/.emacs.19.el#1 $
;;
...
(setq load-path (cons "~/Tools/share/emacs/site-lisp" load-path))
...
(cond (window-system
       ...
       (if t
	     ...
	     ;; in 19.19 since the caching mechanism is broken in 19.19
             ...
             (add-hook 'makefile-mode-hook 'turn-on-face-lock)
             (add-hook 'perl-mode-hook 'turn-on-face-lock)
             (add-hook 'tex-mode-hook 'turn-on-face-lock)
             (add-hook 'latex-mode-hook 'turn-on-face-lock)
