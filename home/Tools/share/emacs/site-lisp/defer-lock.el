...
;; 13-Jul-95|1.00|~/modes/defer-lock.el.Z|
...
(defun defer-lock-submit-bug-report ()
  ...
  (let ((reporter-prompt-for-summary-p t))
    (reporter-submit-bug-report "simon@gnu.ai.mit.edu" "defer-lock 1.00"
...
all of those functions in `defer-lock-defer-post-command-hooks' (if present)."
  ...
  (if (and defer-lock-mode (not font-lock-mode))
    ...
    (defer-lock-hook 'post-command-hook 'defer-lock-post-command-function
