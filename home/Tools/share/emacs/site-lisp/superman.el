...
;; Last Modified:	31-Jul-1991 (AJK)
;;                      27-Jan-1993 (AJK) - Lucid Emacs 19.4
;;                      16-Mar-1993 (AJK) - disable auto section #'s for c/c++ modes
;;                                          translate 3X --> 3
;;
;; Version:		1.1
;;
...
;; |17-Jul-1991|1.0.1|
;; |31-Jul-1991|1.0.2| AJK
;; |27-Jan-1993|1.0.3| AJK
;; |03-Jul-1994|1.1  | AJK
...
(defvar sm-auto-section-alist
  '(
    (c-mode . ("3" "2"))
    (cc-mode . ("3" "2"))
    ;(c++-mode . ("2" "3"))    ; AJK - don't wanna this
    (shell-mode . ("1" "8"))
    (cmushell-mode . ("1" "8"))
    (text-mode . "1")
    )
...
;; AJK: Modified the ^\\([A-Za-z] and added X, Motif and Purdue (for RCS)
...
(defvar sm-filter-list
  '(("sed "
     ("-e 's/.\010//g'"
      ...
      "-e '/^X Version 11/d'"
      "-e '/^Motif [0-9].[0-9a-zA-Z]/d'"
      "-e '/^Purdue University/d'"
      "-e '/^\\n$/D'"
      ))
    ("awk '"
...
(defvar sm-files-regexp "FILES"    ;; AJK
...
;; AJK 03-Jul-94
(defconst sm-man-cmd "man %s 2>/dev/null | ")


;; ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
;; end user variables

(defconst sm-version-number "1.0.3"
...
(if sm-mode-map
  ...
  (define-key sm-mode-map "f"    'sm-goto-files-section)     ;; AJK
...
(defun sm-build-man-command ()
  ...
  ;; AJK replaced prev. line with this:
  (let ((command sm-man-cmd)             ;; AJK 03-Jul-94
...
See the variable sm-notify for the different notification behaviors."
  (cond
   ((eq sm-notify 'bully)
    ...
    ;;(delete-other-windows-quietly))   ;; AJK - unknown in 19.4
    (delete-other-windows))             ;; AJK
...
\\[sm-goto-files-section]       Jumps to the SEE ALSO manpage section.
...
sm-files-regexp                Regexp for FILES section (or your equiv).
...
\\{sm-mode-map}"
  ...
  (setq major-mode 'sm-manual-mode
	...
	truncate-lines t
	buffer-read-only t)
  (if running-lucid			;; JBW
      (buffer-disable-undo (current-buffer)) ;; AJK - only in 19.x
    (buffer-flush-undo (current-buffer))     ;; AJK - obsolete in 19.x
    )
  (auto-fill-mode -1)
  (use-local-map sm-mode-map)
  (run-hooks 'sm-mode-hooks)                 ;; AJK - run hooks
  (goto-char (point-min))
  (sm-build-page-list)
  (sm-goto-page 1))
...
;; AJK
(defun sm-goto-files-section ()
  "Move point the the \"FILES\" section.
Actually the section moved to is described by sm-files-regexp."
  (interactive)
  (if (not (sm-find-section sm-files-regexp))
      (error (concat "No " sm-files-regexp
		     " section found in current manpage."))))
